...
function EditGoalsForm({ record }: { record: MentoringRow }) {
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-1">
      ...
      <input name="goals" defaultValue={record.goals ?? ""} placeholder="Goals" className="rounded-md border border-zinc-300 px-2 py-1 text-xs" />
      <input name="actionPlan" defaultValue={record.action_plan ?? ""} placeholder="Action plan" className="rounded-md border border-zinc-300 px-2 py-1 text-xs" />
      ...
      {canCreate && ownMentorStaffId ? (
        <form action={formAction} className="flex flex-wrap items-end gap-2">
          ...
          <div className="flex-1">
            ...
            <input name="academicObservation" className="w-full rounded-md border border-zinc-300 px-3 py-1.5 text-sm" />
          </div>
          <div className="flex-1">
            <label className="mb-1 block text-xs text-zinc-500">Behaviour observation</label>
            <input name="behaviourObservation" className="w-full rounded-md border border-zinc-300 px-3 py-1.5 text-sm" />
          </div>
          <div className="flex-1">
            <label className="mb-1 block text-xs text-zinc-500">Goals</label>
            <input name="goals" className="w-full rounded-md border border-zinc-300 px-3 py-1.5 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-zinc-500">Confidentiality</label>
            <select name="confidentialityLevel" className="rounded-md border border-zinc-300 px-3 py-1.5 text-sm">
