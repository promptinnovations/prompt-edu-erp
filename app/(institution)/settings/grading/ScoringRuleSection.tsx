        ...
        <td colSpan={5} className="py-2">
          <form action={updateAction} className="flex flex-wrap items-center gap-2" onSubmit={() => setEditing(false)}>
            ...
            <label className="text-xs text-zinc-500">Points
              <input name="points" type="number" step="0.01" defaultValue={rule.points} className="ml-1 w-20 rounded border border-zinc-300 px-2 py-1 text-xs" />
            </label>
            <label className="text-xs text-zinc-500">Max
              <input name="maxPoints" type="number" step="0.01" defaultValue={rule.max_points ?? ""} className="ml-1 w-20 rounded border border-zinc-300 px-2 py-1 text-xs" />
            </label>
      ...
      {canManage ? (
        <form action={createAction} className="flex flex-wrap items-end gap-2 rounded-xl border border-dashed border-zinc-300 p-3">
          <div>
            ...
            <input name="module" required placeholder="reading" className="w-28 rounded border border-zinc-300 px-2 py-1.5 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-zinc-500">Activity code</label>
            <input name="activityCode" required placeholder="fiction_book" className="w-36 rounded border border-zinc-300 px-2 py-1.5 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-zinc-500">Points</label>
            <input name="points" type="number" step="0.01" required className="w-20 rounded border border-zinc-300 px-2 py-1.5 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-zinc-500">Max points</label>
            <input name="maxPoints" type="number" step="0.01" className="w-20 rounded border border-zinc-300 px-2 py-1.5 text-sm" />
          </div>
