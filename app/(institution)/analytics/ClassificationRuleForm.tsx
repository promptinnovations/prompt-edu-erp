  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      <div>
        ...
        <input type="number" step="0.01" name="highThreshold" defaultValue={highThreshold} className="w-28 rounded-md border border-zinc-300 px-3 py-1.5 text-sm" />
      </div>
      <div>
        <label className="mb-1 block text-xs text-zinc-500">Low achiever &lt; (%)</label>
        <input type="number" step="0.01" name="lowThreshold" defaultValue={lowThreshold} className="w-28 rounded-md border border-zinc-300 px-3 py-1.5 text-sm" />
      </div>
