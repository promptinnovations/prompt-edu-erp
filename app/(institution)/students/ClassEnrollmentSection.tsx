    ...
    <div className="space-y-4">
          ...
          {canManage ? (
            <div className="flex flex-wrap items-end gap-4">
              <form action={moveAction} className="flex items-end gap-2">
                ...
                <div>
                  <label className="mb-1 block text-xs text-zinc-500">Move to</label>
                  <select
      ...
      {pastRows.length > 0 ? (
        <details className="text-sm">
          <summary className="cursor-pointer text-zinc-500 underline">
              ...
              <div key={h.id} className="flex items-center justify-between rounded-lg border border-zinc-200 px-3 py-2 text-xs">
                ...
                {canManage ? (
                  <form action={restoreAction}>
                    ...
                    <button type="submit" disabled={restoring} className="text-indigo-600 underline disabled:opacity-50">
          ...
          {restoreState.error ? <p className="mt-1 text-sm text-red-600">{restoreState.error}</p> : null}
        </details>
      ) : null}
    </div>
  );
