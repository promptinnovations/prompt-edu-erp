...
export default async function StudentPortfolioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!can(ctx.permissions, "portfolio.view_all") && !can(ctx.permissions, "portfolio.view_own")) {
    return (
      <div className="space-y-2">
        <p className="text-sm text-zinc-500 dark:text-zinc-400">You don&apos;t have permission to view this student&apos;s portfolio.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <Link href={`/students/${id}`} className="text-sm text-zinc-500 dark:text-zinc-400 underline">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          ...
          {profile.latestResult ? (
            <>
              <div className="mt-2 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{Number(profile.latestResult.percentage).toFixed(1)}%</div>
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          ...
          {profile.attendanceSummary ? (
            <>
              <div className="mt-2 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{profile.attendanceSummary.present_percent}%</div>
              <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5 sm:col-span-2">
          ...
          {profile.latestConsolidatedScore ? (
            <>
              <div className="mt-2 text-2xl font-semibold text-zinc-900 dark:text-zinc-50">{profile.latestConsolidatedScore.score}</div>
              <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">{profile.latestConsolidatedScore.period}</div>
      ...
      <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
        <h2 className="mb-3 text-sm font-semibold text-zinc-700 dark:text-zinc-300">Portfolio timeline</h2>
