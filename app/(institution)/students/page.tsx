    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5">
        <form className="flex flex-wrap items-end gap-2" method="get">
          ...
          <div>
            <label className="mb-1 block text-xs text-zinc-500">{t("filterByClass")}</label>
            <select
              name="classId"
          ...
          {(q || classId || removed) ? (
            <Link href="/students" className="text-xs text-zinc-500 underline">
              {t("clearFilters")}
            </Link>
          ) : null}
        </form>
              ...
              <tr key={s.id} className={s.status === "withdrawn" ? "opacity-50" : undefined}>
                <td className="px-4 py-2">
                  {s.photo_file_id ? (
                    ...
                    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-zinc-100 text-xs font-medium text-zinc-500">
                ...
                <td className="px-4 py-2">
                  {s.full_name}
                  {s.status === "withdrawn" ? <span className="ml-2 text-xs text-red-600">({t("removed")})</span> : null}
                </td>
                <td className="px-4 py-2 text-zinc-500">
                  {s.class_name ? `${s.class_name}${s.section_name ? ` — ${s.section_name}` : ""}` : "—"}
                </td>
                <td className="px-4 py-2 text-zinc-500">{s.roll_number ?? "—"}</td>
