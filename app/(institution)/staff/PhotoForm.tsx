...
import { uploadStaffPhotoAction, removeStaffPhotoAction } from "./actions";

/** §Teacher-Profile feature ("Name & Photo") — same shape as students'
 *  PhotoForm.tsx: a preview (if any), a file picker to replace it, and a
 *  Remove button. */
export default function PhotoForm({ staffId, photoUrl }: { staffId: string; photoUrl: string | null }) {
  const [uploadState, uploadAction, uploadPending] = useActionState<{ error: string | null }, FormData>(
    uploadStaffPhotoAction, { error: null }
  );
  const [removeState, removeAction, removePending] = useActionState<{ error: string | null }, FormData>(
    removeStaffPhotoAction, { error: null }
  );
