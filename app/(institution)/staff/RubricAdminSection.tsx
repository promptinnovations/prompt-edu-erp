  ...
  return (
    <details className="rounded-lg border border-zinc-200 p-3">
      ...
      <form action={delAction} className="mt-2">
        <input type="hidden" name="criterionId" value={criterion.id} />
        <button type="submit" disabled={delPending} className="text-xs text-red-600 underline disabled:opacity-50">
          Delete this criterion
        </button>
        {delState.error ? <span className="ml-2 text-xs text-red-600">{delState.error}</span> : null}
      </form>
    </details>
  );
}
