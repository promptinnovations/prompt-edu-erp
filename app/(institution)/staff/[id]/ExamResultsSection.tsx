  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-zinc-200 bg-white p-5">
        <h2 className="mb-3 text-sm font-semibold text-zinc-700">Performance trend</h2>
        ...
        {!report ? (
          ...
          <>
            <div className="mb-3 flex flex-wrap items-center gap-4 text-sm">
              <span className="font-semibold text-zinc-900">{report.examination_name}</span>
              <span className="text-zinc-500">
                Overall: <span className="font-medium text-zinc-900">{report.overall_percentage ?? "—"}%</span>
              </span>
              <span className="text-zinc-500">
                Pass <span className="font-medium text-zinc-900">{report.overall_pass_percentage ?? "—"}%</span>
              </span>
            </div>
                ...
                <tbody className="divide-y divide-zinc-100">
                  {report.rows.map((r) => (
                    <tr key={`${r.class_id}-${r.section_id ?? "all"}-${r.subject_id}`}>
                      ...
                      <td className="py-1.5 pr-3 text-zinc-600">{r.subject_name}</td>
                      <td className="py-1.5 pr-3 text-zinc-600">{r.students}</td>
                      <td className="py-1.5 pr-3 text-zinc-600">
                        {r.average_marks !== null ? `${r.average_marks}/${r.full_marks}` : "—"}
                      </td>
                      <td className="py-1.5 pr-3 text-zinc-600">{r.pass_percentage !== null ? `${r.pass_percentage}%` : "—"}</td>
                      <td className="py-1.5 pr-3 text-zinc-600">{r.full_marks}</td>
                      {gradeLabels.map((g) => (
                        <td key={g} className="py-1.5 pr-3 text-zinc-600">{r.grade_counts[g] ?? 0}</td>
                      ))}
                      <td className="py-1.5 pr-3 text-zinc-600">{r.failed_count}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
