      ...
      {canManage && remaining.length > 0 ? (
        <form action={assignAction} className="mt-1 flex items-center gap-2">
          ...
          <select
            name="subjectId"
            required
            className="rounded-full border px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-400 focus:border-indigo-400"
          >
            {remaining.map((s) => (
              <option key={s.id} value={s.id}>{s.name}</option>
            ))}
          </select>
          <button type="submit" disabled={assignPending} className="rounded-full border px-2 py-1 text-xs text-zinc-700 hover:bg-zinc-100 disabled:opacity-50">
            Add subject
          </button>
        </form>
      ) : null}
