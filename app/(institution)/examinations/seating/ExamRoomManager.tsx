...
export default function ExamRoomManager({ rooms }: { rooms: ExamRoomRecord[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {deleteState.error ? <p className="text-sm text-red-600 dark:text-red-400">{deleteState.error}</p> : null}
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {rooms.map((room) =>
              editingId === room.id ? (
                ...
                <tr key={room.id}>
                  <td className="px-3 py-2 font-medium text-zinc-800 dark:text-zinc-100">{room.name}</td>
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="flex justify-end gap-3">
                      <button type="button" onClick={() => setEditingId(room.id)} className="text-sm text-zinc-600 dark:text-zinc-400 underline">
                        Edit
                      </button>
                      <form action={deleteAction}>
                        <input type="hidden" name="roomId" value={room.id} />
                        <button type="submit" className="text-sm text-red-600 dark:text-red-400 underline">Delete</button>
                      </form>
                    </div>
                  </td>
                </tr>
            ...
            {rooms.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-3 py-6 text-center text-zinc-400 dark:text-zinc-500">
                  No rooms yet — add the classrooms you use for examinations, once, and reuse them for every exam.
                </td>
              </tr>
            ) : (
              <tr className="bg-zinc-50 dark:bg-zinc-950">
                <td className="px-3 py-2 text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400" colSpan={3}>Total capacity</td>
                <td className="px-3 py-2 font-semibold text-zinc-800 dark:text-zinc-100">{totalCapacity}</td>
...
function EditRoomRow({ room, onDone }: { room: ExamRoomRecord; onDone: () => void }) {
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      <button type="button" onClick={onDone} className="rounded-lg border border-zinc-300 px-3 py-1.5 text-sm text-zinc-600 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800">
        Cancel
      </button>
