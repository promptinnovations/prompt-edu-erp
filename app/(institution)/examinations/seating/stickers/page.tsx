  ...
  return (
    <div className="space-y-6">
      <div className="no-print flex flex-wrap items-center justify-between gap-3">
        <Link href={`/examinations/seating?examinationId=${examinationId}`} className="text-sm text-zinc-500 underline dark:text-zinc-400">
          ← Back to Seating Arrangement
        </Link>
        <PrintButton label="Print stickers" />
      </div>

      <h1 className="no-print text-2xl font-semibold text-zinc-900 dark:text-zinc-50">
        ...
        <section className="print-area rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          {byRoom.map((room) => (
            <div key={room.roomName} className="mb-6">
              <h2 className="mb-2 border-b border-zinc-200 pb-1 text-sm font-semibold text-zinc-800 dark:border-zinc-800 dark:text-zinc-100">
                {room.roomName}
              </h2>
                ...
                <div key={bench.benchNumber} className="mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {bench.seats.map((seat) => (
                      <div
                        key={seat.id}
                        className="break-inside-avoid rounded-lg border border-dashed border-zinc-400 p-2.5 dark:border-zinc-600"
                      >
                        <p className="text-sm font-semibold leading-tight text-zinc-900 dark:text-zinc-50">{seat.student_name}</p>
                        ...
                        <p className="mt-1 border-t border-zinc-200 pt-1 text-[11px] text-zinc-500 dark:border-zinc-700 dark:text-zinc-400">
